...
def get_center(mask):
    ...
    for x in np.arange(mask.shape[0]):
        for y in np.arange(mask.shape[1]):
            if mask[x][y] == 1:
                allp.append((y,x))
                # xs.append(x)
                # ys.append(y)

    # allp = np.sort(np.asarray(allp))
    return allp[int(len(allp)/2)]
    # xs = np.sort(np.asarray(xs))
    # ys = np.sort(np.asarray(ys))
#     print("Median x {}, y {}".format(xs[int(xs.size/2)], ys[int(ys.size/2)]))
    # return (int(np.average(ys)), int(np.average(xs)))
    # return (ys[int(ys.size/2)], xs[int(xs.size/2)])


def get_prediction_with_centers(encoded_image):
    dec = get_decoded_image(encoded_image)
    img = transforms.ToTensor()(dec)
    ...
    with torch.no_grad():
        ...
        for i in range(N):
            ...
            if prediction[0]['scores'][i] > thresh:
              mask = Image.fromarray(prediction[0]['masks'][i, 0].mul(255).byte().cpu().numpy())
              masked_image = apply_mask(masked_image, mask.copy(), color)
              centers.append(get_center(mask.copy()))

        masked = Image.fromarray(masked_image.astype('uint8'), 'RGB')

    return masked, centers, colors
