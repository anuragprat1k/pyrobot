...
endpoint_D = "http://192.168.1.6:5000/segment_orient"
...
def encode_and_infer_D(rgb, pts):
	# encode and infer
    img = encode_img(rgb)
    # encode 3dpts
    dpts = base64.b64encode(pts)
    payload = {'encoded_image' : img, 'point_cloud' : dpts}
    resp = requests.post(endpoint_D, files = payload)
    print(resp.text)
